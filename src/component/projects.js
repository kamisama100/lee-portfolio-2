...
export default function Projects() {
  ...
  return (
    <div className='flex flex-col w-[100%] mx-auto gap-y-[7rem]' id = "projects">
        {
            proj.map((proj,index)=>(
                <div className='relative flex flex-row w-[100%] mx-auto'>
                    <div className='flex w-[850px] h-[500px] bg-[#17181D] transition-transform hover:scale-[0.95] duration-200 hover:cursor-pointer '>
                    <img src = {proj.img} className = " w-[85%] h-[380px] my-auto" alt = "todoapp"/>
                    </div>
                    <div className='absolute right-[5rem] top-[3rem] w-[350px] text-center space-y-5'>
                        <h3 className='font-semibold'>{proj.title}</h3>
                        <p className='text-[18px]'>
                            {proj.text}
                        </p>
                        <button className='btn inline-block border-b-[4px] w-[60%] border-[#FDB750]'>Go to App</button>
                        <button className='btn inline-block border-b-[4px] w-[60%] border-[#FDB750]' >Go to Repo</button>
                    </div>
                </div>
            ))
        }
